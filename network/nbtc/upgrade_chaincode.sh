...
docker exec -e CC_VERSION= -t $(docker ps -q -f name=cli_peer0.nbtc.mobileid.com) bash scripts/installChaincode.sh

echo "Upgrading new chaincode ..."
docker exec -e CC_VERSION= -t $(docker ps -q -f name=cli_peer0.nbtc.mobileid.com) bash scripts/upgradeChaincode.sh

echo "Please verify chaincode digest ..."
docker exec -t $(docker ps -q -f name=cli_peer0.nbtc.mobileid.com) peer chaincode list --installed
docker exec -t $(docker ps -q -f name=cli_peer0.nbtc.mobileid.com) peer chaincode list --instantiated -C mobileid
