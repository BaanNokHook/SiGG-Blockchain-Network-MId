...
export CFG_PATH=~/crypto-config/peerOrganizations/nbtc.mobileid.com/ca
export FABRIC_CA_CLIENT_TLS_CERTFILES=~/crypto-config/peerOrganizations/nbtc.mobileid.com/ca/ca-cert.pem
...
if [ -z "$affiliation" ]
then
  read -p "Required affiliation (nbtc.department1): " affiliation
  echo
fi
if [ "$role" == "admin" ]
then
